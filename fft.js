  ...
  try {
    ...
    do {
      ...
      for (let i = 0; i < TargetResult.length; i++) {
	    ...
	    var Text = fs.readFileSync('komen.txt', 'utf8').split('|');            var ranText = Text[Math.floor(Math.random() * Text.length)];

	    var iki = ranText+' '+akun.params.username;

            const ngeDo = await CommentAndLike(doLogin.session, akun.id, iki)

            console.log(chalk`[{magenta ${timeNow}}] [${User.username}] {bold.green [>]}${akun.params.username} => ${ngeDo}`)

          } else {

            console.log(chalk`[{magenta ${timeNow}}] [${User.username}] {bold.yellow [SKIP]}${akun.params.username} => PRIVATE OR ALREADY FOLLOWED`)
...
console.log(chalk`
  ...
  [?] {bold.green TOOLS PROMOSI IG ONLY KOMEN DAN LIKE}
  ...
  ====================PRIVATE TOOLS====================
  			...
  			TOOLS UNTUK PROMOSI AKUN BISNIS
  				RECODE BY SI TAMPAN
